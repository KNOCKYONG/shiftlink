-- ==============================================
-- Monitoring & Analytics Schema for ShiftLink (FIXED)
-- ==============================================
-- Purpose: Track work hours, fatigue, performance metrics
-- Phase 8: Monitoring & Reports Implementation
-- ==============================================

-- First, ensure schedule_assignments has required columns
...
SET shift_date = date 
WHERE shift_date IS NULL AND date IS NOT NULL;
...
CREATE TABLE IF NOT EXISTS work_time_aggregations (
  ...
  -- Compliance metrics
  weekly_limit_violations INTEGER DEFAULT 0,
  rest_time_violations INTEGER DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS team_balance_reports (
  ...
  -- Balance scores
  seniority_balance_score DECIMAL(5,2) DEFAULT 0,
  skill_balance_score DECIMAL(5,2) DEFAULT 0,
  shift_balance_score DECIMAL(5,2) DEFAULT 0,
  overall_balance_score DECIMAL(5,2) DEFAULT 0,
...
BEGIN
  ...
  WHERE sa.employee_id = p_employee_id
    ...
    AND sa.shift_date > p_date - INTERVAL '7 days';
  
  -- Get consecutive work days
  SELECT COUNT(DISTINCT sa.shift_date)
  INTO v_consecutive_days
  FROM schedule_assignments sa
  WHERE sa.employee_id = p_employee_id
    AND sa.shift_type != 'off'
    AND sa.shift_date <= p_date
    AND sa.shift_date > p_date - INTERVAL '14 days';
...
BEGIN
  -- Only process confirmed assignments
...
-- Schedule assignments indexes (for the new columns)
CREATE INDEX IF NOT EXISTS idx_schedule_assignments_shift_date ON schedule_assignments(shift_date);
...
CREATE INDEX IF NOT EXISTS idx_schedule_assignments_employee_date ON schedule_assignments(employee_id, shift_date);
