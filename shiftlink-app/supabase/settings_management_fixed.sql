...
BEGIN
  ...
  IF v_preset.constraints IS NOT NULL THEN
    ...
    FOR v_constraint_item IN SELECT * FROM jsonb_array_elements(v_preset.constraints)
    LOOP
      INSERT INTO constraint_settings (tenant_id, constraint_type, constraint_name, config)
      VALUES (
        p_tenant_id,
        (v_constraint_item->>'type')::VARCHAR,
        (v_constraint_item->>'name')::VARCHAR,
        v_constraint_item->'config'
      )
...
CREATE OR REPLACE FUNCTION track_setting_changes() RETURNS TRIGGER AS $FUNC$
BEGIN
...
CREATE INDEX IF NOT EXISTS idx_constraint_settings_tenant ON constraint_settings(tenant_id);
CREATE INDEX IF NOT EXISTS idx_constraint_settings_type ON constraint_settings(constraint_type, is_enabled);
CREATE INDEX IF NOT EXISTS idx_schedule_rules_tenant ON schedule_rules(tenant_id, is_active);
CREATE INDEX IF NOT EXISTS idx_schedule_rules_team ON schedule_rules(team_id) WHERE team_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_simulation_configs_tenant ON simulation_configs(tenant_id, status);
CREATE INDEX IF NOT EXISTS idx_setting_presets_type ON setting_presets(preset_type, is_public);
CREATE INDEX IF NOT EXISTS idx_setting_history_tenant ON setting_history(tenant_id, created_at DESC);
...
-- Insert default constraint settings for existing tenants
...
FROM tenants
ON CONFLICT (tenant_id, constraint_type, constraint_name) DO NOTHING;
