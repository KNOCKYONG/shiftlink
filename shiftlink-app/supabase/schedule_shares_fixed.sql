-- Fixed Schedule Shares SQL with proper dependencies and syntax
-- Ensure schedules and schedule_versions tables exist first
...
-- Create schedule_versions table if it doesn't exist
...
-- Now create the schedule_shares table
...
-- 트리거: 공유 생성 시 통계 업데이트 (Fixed with $$)
...
-- Verify creation
SELECT 
    'schedule_shares tables created successfully' as status
WHERE EXISTS (
    SELECT 1 FROM information_schema.tables 
    WHERE table_name IN ('schedule_shares', 'share_access_logs')
);